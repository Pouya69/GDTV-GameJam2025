...
public partial class FindSpotAroundLocationAction : Action
{
    ...
    [NonSerialized] private List<FEQS_Result> EQS_Results = new();

    protected override Status OnStart()
    {
        bool EQS_Result_Success = RunCheck();  // Successful if found a point, if not false.
    ...
    public bool RunCheck()
    {
        ...
        Vector3 StartLocation = IsStartPointVector3.Value ? StartPointVector3.Value : StartPointTransform.Value.position;
        ...
        for (int x = 0; x <= stepsX; x++)
        {
            for (int y = 0; y <= stepsY; y++)
            {
                for (int z = 0; z <= stepsZ; z++)
                {
                    ...
                    Vector3 TargetLocationCheck = StartLocation + Offset;
